...
func doMake(arg2, arg3 string) error {
	...
	case "migration":
		...
		upFile := nav.RootPath + "/migrations/" + fileName + "." + dbType + ".up.sql"
		downFile := nav.RootPath + "/migrations/" + fileName + "." + dbType + ".down.sql"
	...
	case "handler":
		...
		fileName := nav.RootPath + "/handlers/" + strings.ToLower(arg3) + ".go"
	...
	case "model":
		...
		fileName := nav.RootPath + "/data/" + strings.ToLower(modelName) + ".go"
		if fileExists(fileName) {
			exitGracefully(errors.New(fileName + " already exists!"))
		}
